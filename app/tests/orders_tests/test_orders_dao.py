...
async def test_make_order_success(async_client: AsyncClient, auth_headers):
    async with async_session_maker() as session:
        ...
        initial_stock = {row["product_id"]: row["product_quantity"] for row in stock_result.mappings().all()}
    ...
    async with async_session_maker() as session:
        stock_result = await session.execute(stock_query)
        new_stock = {row["product_id"]: row["product_quantity"] for row in stock_result.mappings().all()}
